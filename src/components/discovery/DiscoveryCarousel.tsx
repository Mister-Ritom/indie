...
import { View, Text, FlatList, Dimensions, NativeSyntheticEvent, NativeScrollEvent, TouchableOpacity } from 'react-native';
...
export function DiscoveryCarousel({ pins, isLoading }: DiscoveryCarouselProps) {
  ...
  if (isLoading || pins.length === 0) {
    return (
      <View style={{ height: CARD_HEIGHT, marginHorizontal: spacing.md, backgroundColor: colors.surface, borderRadius: radius.lg, justifyContent: 'center', alignItems: 'center' }}>
  ...
  return (
    <View style={{ marginBottom: spacing.xl }}>
      <FlatList
        ...
        renderItem={({ item }) => {
          const mainAsset = item.assets?.find(a => a.variant === '720' || a.variant === 'original') || item.assets?.[0];
          
          return (
            <TouchableOpacity 
              activeOpacity={0.9} 
              onPress={() => router.push(`/pin/${item.id}`)}
              style={{ width: CARD_WIDTH, height: CARD_HEIGHT, marginRight: spacing.md, borderRadius: radius.xl, overflow: 'hidden' }}
            >
              <Image
