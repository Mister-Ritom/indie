...
export default function SavedPinsScreen() {
  ...
  const [isPrivate, setIsPrivate] = useState(profile?.all_saves_private ?? true);
          ...
          ListHeaderComponent={
            <View style={{ padding: spacing.xl, paddingBottom: spacing.lg }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: spacing.lg }}>
                ...
                {isCurrentUser && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: colors.surface, padding: spacing.sm, borderRadius: 12 }}>
                    {isPrivate ? <Lock size={16} color={colors.icon} /> : <Globe size={16} color={colors.icon} />}
                    <Text style={{ fontFamily: typography.families.bodyMedium, fontSize: typography.scale.bodySmall, color: colors.text }}>
