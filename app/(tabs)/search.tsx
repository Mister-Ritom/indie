...
import { View, Text, TextInput, TouchableOpacity, ActivityIndicator, useWindowDimensions, ScrollView } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Search as SearchIcon, X, Camera, ChevronRight } from 'lucide-react-native';
...
export default function SearchScreen() {
  ...
  const renderItem = useCallback(({ item }: { item: FeedPin }) => {
    return (
              ...
              profileResults.length > 0 ? (
                <View style={{ marginBottom: spacing.xl, marginTop: spacing.md }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.md, paddingHorizontal: spacing.md }}>
                    <Text style={{ fontFamily: typography.families.headingBold, fontSize: 20, color: colors.text }}>
                      Users
                    </Text>
                    <TouchableOpacity 
                      onPress={() => router.push(`/search/users?q=${encodeURIComponent(debouncedQuery)}`)}
